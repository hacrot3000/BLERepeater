...
class RealGattCaptureManager(private val context: Context) {
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            Log.d("RealGattCaptureManager", "onConnectionStateChange: status=$status, newState=$newState")
        ...
        override fun onCharacteristicRead(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
            ...
            Log.d("RealGattCaptureManager", "onCharacteristicRead: ${characteristic.uuid}, status=$status")
            if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        override fun onDescriptorRead(gatt: BluetoothGatt, descriptor: BluetoothGattDescriptor, status: Int) {
            ...
            Log.d("RealGattCaptureManager", "onDescriptorRead: ${descriptor.uuid}, status=$status")
            if (status == BluetoothGatt.GATT_SUCCESS) {
    ...
    private suspend fun captureFullProfile(gatt: BluetoothGatt) {
        ...
        for (service in services) {
            ...
            for (char in service.characteristics) {
                ...
                capturedChars.add(CapturedGattCharacteristic(
                    uuid = char.uuid.toString(),
                    properties = char.properties,
                    permissionsForServer = char.permissions, // Tạm thời lấy permission này
                    value = value,
            ...
            capturedServices.add(CapturedGattService(
        ...
        val profile = CapturedBleProfile(
            deviceName = device.name,
            deviceAddress = device.address,
            lastRssi = 0, // Sẽ cập nhật sau nếu cần
    ...
    @SuppressLint("MissingPermission")
    private suspend fun readCharacteristicSuspending(gatt: BluetoothGatt, char: BluetoothGattCharacteristic): ByteArray? = mutex.withLock {
    ...
    @SuppressLint("MissingPermission")
    private suspend fun readDescriptorSuspending(gatt: BluetoothGatt, desc: BluetoothGattDescriptor): ByteArray? = mutex.withLock {
        operationDeferred = CompletableDeferred()

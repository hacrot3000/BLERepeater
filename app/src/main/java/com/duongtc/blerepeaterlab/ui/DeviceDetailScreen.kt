...
fun DeviceDetailScreen(viewModel: MainViewModel) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        if (profile == null) {
            ...
            val p = profile!!
            Text(text = "Thiết bị: ${p.deviceName ?: "Không rõ tên"}", style = MaterialTheme.typography.titleLarge)
            ...
            Row(modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp)) {
                ...
                Button(
                    ...
                    Text("Disconnect Real")
                }
            }

            LazyColumn(modifier = Modifier.weight(1f)) {
                items(p.services) { service ->
                    ServiceItem(service)
                }
            }
        }
...
fun CharacteristicItem(char: CapturedGattCharacteristic) {
    Column(modifier = Modifier.padding(start = 16.dp, top = 4.dp)) {
        ...
        Text(text = "Properties: ${getPropertiesString(char)}", style = MaterialTheme.typography.bodySmall)
        ...
        char.descriptors.forEach { desc ->
            Text(
                text = "  Desc: ${desc.uuid} -> ${desc.value?.let { BlePayloadUtils.bytesToHex(it) } ?: "None"}",
